class StaticPagesController < ApplicationController
  ...
  def notify
    if mac_value_ok?
      ...
      Mail.deliver(from:'LINE DDD',to: Register.find_by(email: Transaction.find_by(trade_number: params[:MerchantTradeNo]).register_id).email,subject:"3D列印競賽繳費成功通知",body:"您已於歐付寶成功支付LINE DDD主辦3D列印競賽報名費用")
    end
  end
